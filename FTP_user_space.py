...
HOST = '10.1.2.3' #'10.1.1.3'  # Standard loopback interface address (localhost)
HOST_OUT = '10.1.1.3' #'10.1.2.3'
PORT = 210        # Port to listen on (non-privileged ports are > 1023)
pattern = "PORT \d{1,*}.\d{1,*}.\d{1,*}.\d{1,*}.\d{1,*}.\d{1,*}\r\n"
    ...
    with client_socket:
        ...
        with open('/sys/class/fw/conns/conns', 'r') as connections:
            ...
            while (line):
                ...
                line = line.replace("\n", "")
                conn1 = line.split()
                if (get_ip_addr_without_mask(int(conn1[0])) == addr[0] and conn1[2] == str(addr[1]) and conn1[3] == "21"):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
            ...
            with open('/sys/class/fw/mitm/mitm', 'w') as mitm_dev:
                sen = "{} {} {} {} {} {}".format(parse_ip_addr(addr[0]), addr[1], parse_ip_addr(str(dst_ip)), 21, parse_ip_addr(HOST_OUT), MITM_PORT)
                print(sen)
                mitm_dev.write(sen)
            server_socket.settimeout(10)
            try:
                server_socket.connect((dst_ip, 21))
            except:
                server_socket.close()
                client_socket.close()
                return
            ...
            while True:
                ...
                if not data_c == 1:
                    data_c_str = data_c.decode('utf-8')
                    port_cmd = re.search(pattern, data_c_str)
                    if port_cmd:
                        arr = port_cmd.split('.')
                        port = int(arr[4])*256 + int(arr[5])
                        ip = parse_ip_addr(".".join([arr[0].split()[1], arr[1], arr[2], arr[3]]))
                        with open('/sys/class/fw/mitm/mitm', 'w') as mitm_dev:
                            sen = "{} {} {} {}".format(parse_ip_addr(str(dst_ip)), 20, ip, port)
                            print(sen)
                            mitm_dev.write(sen)
